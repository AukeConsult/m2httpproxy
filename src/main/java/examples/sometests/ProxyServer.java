...
public class ProxyServer {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// tuples. Create an appropriate Proxy object, and add it to the
			// server
			int i = 0;
			while (i < args.length) {
				String host = args[i++];
		...
		} catch (Exception e) { // Print an error message if anything goes
								// wrong.
	...
	public static class Proxy implements IService {
		...
		public void serve(InputStream in, OutputStream out) {
			...
			// These are some sockets we'll use. They are final so they can be
			// used
			...
			// instead of two variables because given the structure of this
			// program
			// two variables would not work if declared final.
			final Thread[] threads = new Thread[2];

			// Define and create a thread to transmit bytes from client to
			// server
			Thread c2s = new Thread() {
				...
				public void run() {
					...
					// if the client closed its stream to us, we close our
					// stream
			...
			Thread s2c = new Thread() {
				...
				public void run() {
					...
					// if the server closed its stream to us, we close our
					// stream
